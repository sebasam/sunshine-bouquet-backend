...
class ComposeDirectiveManager {
    ...
    getLatestIfCompatible(coreIdentity, subgraphsUsed) {
        ...
        return {
            ...
            subgraphNames: featureUrlsAndSubgraphs
                .filter(([feature]) => feature.url.version.equals(latest.feature.url.version))
                ...
                if (directive) {
                    ...
                    if (featureDetails) {
                        ...
                        if (featureDetails.nameInFeature === null) {
                            this.pushError(federation_internals_1.ERRORS.DIRECTIVE_COMPOSITION_ERROR.err(`Directive "@${name}" in subgraph "${sg.name}" cannot be composed because it is imported to a different name`, { nodes: composeInstance.sourceAST }));
                        }
    ...
    getLatestDirectiveDefinition(directiveName) {
        ...
        if (val) {
            ...
            this.pushError(federation_internals_1.ERRORS.DIRECTIVE_COMPOSITION_ERROR.err(`Core feature "${identity}/${feature.url.version}" in ${(0, federation_internals_1.printSubgraphNames)(subgraphNames)} ${plural ? 'do' : 'does'} not have a directive definition for "@${directiveName}"`, {
                nodes: sourceASTs,
            }));
        }
        return undefined;
