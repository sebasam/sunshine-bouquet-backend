...
class Merger {
    constructor(subgraphs, options) {
        ...
        const { subgraphNamesToJoinSpecName, importConflictsByIdentity, computeUniqueAlias, } = this.prepareSupergraph();
    ...
    prepareSupergraph() {
        ...
        const specAliases = [];
        for (const feature of (_b = (_a = this.merged.coreFeatures) === null || _a === void 0 ? void 0 : _a.allFeatures()) !== null && _b !== void 0 ? _b : []) {
            specAliases.push({
                url: feature.url,
        ...
        for (const spec of federation_internals_1.FEDERATION_OPERATION_TYPES) {
        ...
        const { importConflictsByIdentity, computeUniqueAlias, } = federation_internals_1.CoreFeatures.computeAliasConflicts(specAliases, elementNames);
        this.maybeAddSpecs(supergraphInfoByIdentity, importConflictsByIdentity, computeUniqueAlias);
    ...
    validateCoreDirectiveInfo(directivesMergeInfo) {
    ...
    maybeAddSpecs(supergraphInfoByIdentity, importConflictsByIdentity, computeUniqueAlias) {
        var _a, _b, _c;
        for (const { specInSupergraph, directives } of supergraphInfoByIdentity.values()) {
            const specAlias = ((_a = this.merged.coreFeatures) === null || _a === void 0 ? void 0 : _a.isAliasValid(specInSupergraph.url.name, specInSupergraph.identity, importConflictsByIdentity))
            ...
            for (const { nameInFeature, nameInSupergraph, compositionSpec } of directives) {
                ...
                const defaultNameInSupergraph = federation_internals_1.CoreFeature.directiveNameInSchemaForCoreArguments(specInSupergraph.url, specAlias, [], nameInFeature);
            ...
            try {
                elementAdditionErrors = this.linkSpec.applyFeatureToSchema(this.merged, specInSupergraph, specAlias === specInSupergraph.url.name ? undefined : specAlias, specInSupergraph.defaultCorePurpose, imports);
            }
            catch (e) {
            ...
            (0, federation_internals_1.assert)(elementAdditionErrors.length === 0, "We shouldn't have errors adding spec elements to the (still empty) supergraph schema");
            const feature = (_b = this.merged.coreFeatures) === null || _b === void 0 ? void 0 : _b.getByIdentity(specInSupergraph.url.identity);
    ...
    addCoreFeatures() {
        ...
        for (const [feature, directives] of features) {
            const specAlias = ((_a = this.merged.coreFeatures) === null || _a === void 0 ? void 0 : _a.isAliasValid(feature.url.name, feature.url.identity, this.importConflictsByIdentity))
                ? feature.url.name
    ...
    hintOnInconsistentValueTypeField(sources, dest, field) {
        ...
        for (const [index, source] of sources.entries()) {
            if (source && (0, federation_internals_1.isObjectType)(source) && this.metadata(index).isInterfaceObjectType(source)) {
                continue;
            }
            if (source && !source.field(field.name) && !this.areAllFieldsExternal(index, source)
                && !((0, federation_internals_1.isInterfaceType)(dest) && this.isFieldProvidedByAnInterfaceObject(field.name, dest.name))) {
                this.mismatchReporter.reportMismatchHint({
                    code: hintId,
        ...
        for (const [idx, source] of sources.entries()) {
            ...
            for (const directive of source.appliedDirectives) {
                ...
                if (sourceFeature &&
                    ...
                    sourceFeature.feature.url.identity == federation_internals_1.linkIdentity) {
